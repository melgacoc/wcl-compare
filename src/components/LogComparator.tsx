...
interface PlayerData { name: string; id: number; type: string; icon: string; total: number; activeTime: number; }
...
export function LogComparator() {
  ...
  const [getDataA, { data: dataA, loading: loadA }] = useLazyQuery(GET_REPORT_DATA);
  const [getDataB, { data: dataB, loading: loadB }] = useLazyQuery(GET_REPORT_DATA);
  ...
  const processList = (data: any) => {
    ...
    const entries: PlayerData[] = data.reportData.report.table.data.entries;
