...
interface AbilityData {
  name: string;
  total: number; // Dano/Cura total
  uses: number;  // Casts/Hits
  icon: string;  // Nome do ícone (ex: spell_fire_fireball)
}

export function AbilityComparator({ playerA, reportA, playerB, reportB, dataType }: AbilityComparatorProps) {
  // Busca habilidades do Jogador A
  const { data: dataA, loading: loadA } = useQuery(GET_PLAYER_DETAILS, {
    variables: { code: reportA.code, fightId: reportA.fightId, playerId: playerA.id, dataType }
  });

  // Busca habilidades do Jogador B
  const { data: dataB, loading: loadB } = useQuery(GET_PLAYER_DETAILS, {
  ...
  // Criar lista única de habilidades (Merge pelo nome)
  ...
  // Calcula totais para ordenação (quem deu mais dano na skill mais forte aparece primeiro)
  const comparisonList = allAbilities.map(name => {
    const skillA = mapA.get(name);
    const skillB = mapB.get(name);
    
    // Critério de ordenação: Maior dano entre os dois
